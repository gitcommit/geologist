...
class ConnectionData {
 public:
  ConnectionData(const QString& host = QString::null,
  ...
  QString database() const { return db_; }
  ...
  QString info() const { return QString("QPSQL7://%1@%2:%3/%4").arg(login()).arg(host()).arg(port()).arg(database()); }

  void setHost(const QString& h) { host_ = h; }
  void setPort(const QString& p) { port_ = p; }
  void setDatabase(const QString& d) { db_ = d; }
  void setLogin(const QString& l) { login_ = l; }
  void setPassword(const QString& p) { pass_ = p; }
  void setDrop(const bool& d) { drop_ = d; }
  void setCreate(const bool& c) { create_ = c; }
  void setDebug(const bool& d) { debug_ = d; }
 private:
  QString host_;
  QString port_;
  QString db_;
